...
func setFlags() {
	...
	flag.Parse()
	cmdFlags["port"] = strconv.Itoa(*portFlag)
	cmdFlags["mode"] = *envModeFlag
	cmdFlags["simulator"] = *simulatorFlag
}
...
func getConfig(update bool) Configuration {
	...
	if cmdFlags["port"] != "0" {
		go api.Start(cmdFlags["port"].(string), GetDirectory())
	} else {
		//ask
